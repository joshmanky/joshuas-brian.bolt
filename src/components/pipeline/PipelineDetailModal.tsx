// PipelineDetailModal: expanded card detail with tabs (Skript, Caption, Media, Performance)
// Updated: added media tab, performance fields, tabbed layout
import { useState, useEffect } from 'react';
import { ExternalLink, Copy, Check, Calendar, Hash, Film, BarChart3, FileText, Save } from 'lucide-react';
...
export default function PipelineDetailModal({ card, onClose, onUpdated }: PipelineDetailModalProps) {
  ...
  useEffect(() => {
    if (card) {
      setScheduledDate(card.scheduled_date?.split('T')[0] || '');
        ...
        {activeTab === 'performance' && (
          <div className="space-y-3">
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-xs font-medium text-jb-text-secondary mb-1">Views (48h)</label>
                <input
                  type="number"
                  value={views48h}
                  onChange={(e) => setViews48h(e.target.value)}
                  min="0"
              ...
              <div>
                <label className="block text-xs font-medium text-jb-text-secondary mb-1">Likes (48h)</label>
                <input
                  type="number"
                  value={likes48h}
                  onChange={(e) => setLikes48h(e.target.value)}
                  min="0"
                  className="w-full bg-jb-bg border border-jb-border rounded-lg px-3 py-2 text-sm text-jb-text focus:outline-none focus:border-jb-accent/50 transition-colors"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-jb-text-secondary mb-1">Watchtime</label>
                <input
                  type="number"
                  value={watchtimeScore}
                  onChange={(e) => setWatchtimeScore(e.target.value)}
                  min="0"
                  max="100"
                  className="w-full bg-jb-bg border border-jb-border rounded-lg px-3 py-2 text-sm text-jb-text focus:outline-none focus:border-jb-accent/50 transition-colors"
                />
